    ...
    <main className="pt-32 pb-24 px-6 max-w-7xl mx-auto">
      ...
      <div className="flex flex-col lg:flex-row gap-8 mb-16 bg-white dark:bg-zinc-900 p-10 rounded-2xl border border-primary/10">
        <div className="flex flex-col items-center lg:items-start lg:w-1/3">
          <p className="text-7xl font-black text-secondary dark:text-primary mb-2 tracking-tighter">4.9</p>
          <div className="flex text-primary mb-2">
            {[1, 2, 3, 4, 5].map(i => <span key={i} className="material-icons text-xl">star</span>)}
          </div>
          <p className="text-xs font-bold uppercase tracking-widest opacity-40">Based on 1,240 reviews</p>
        ...
        <div className="flex-1 space-y-4">
          {[
            ...
            <div key={row.star} className="flex items-center gap-4">
              <span className="text-xs font-bold w-4">{row.star}</span>
              <div className="flex-1 h-2 bg-primary/10 rounded-full overflow-hidden">
                <div className="h-full bg-primary" style={{ width: `${row.pct}%` }}></div>
              </div>
              <span className="text-xs opacity-40 w-8">{row.pct}%</span>
            </div>
          ))}
        </div>
      </div>
